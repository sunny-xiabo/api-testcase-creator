---
name: source-command-api-testcase-creator
description: 从 OpenAPI/Swagger 接口文档自动生成测试用例和 pytest 自动化代码
---

# source-command-api-testcase-creator

Use this skill when the user asks to run the migrated source command `api-testcase-creator`.
...
### 3.2 多 reviewer 独立评审（轻量模式）

目标：提高评审准确性，但避免上下文膨胀。不要把评审做成平台；只在当前 Skill 流程内完成。

**上下文控制规则（必须遵守）：**

- 若接口数 > 10，先按 tag/module 分批评审，每批最多 10 个接口。
- 每个 reviewer 只关注自己的职责，不重复输出其他 reviewer 的结论。
- 每个 reviewer 最多输出 5 条发现、5 条建议新增/修改用例、3 条不适用项。
- 每条原因控制在 1-2 句。
- 不输出大段解释，不复制完整用例表。
- 第二轮评审只有在用户明确补充要求时触发，且只围绕用户反馈复核，不重复全量评审。

**默认 reviewer 角色：**

| Reviewer | 关注点 | 对应维度 |
|----------|--------|----------|
| 参数边界 reviewer | 必填、类型、边界、枚举、pattern | PARAM |
| 鉴权权限 reviewer | 无 Token、伪造 Token、越权、多角色 | AUTH |
| 业务流程 reviewer | 业务规则、状态流转、跨接口链路 | BIZ / CHAIN |
| 数据断言 reviewer | 响应字段、错误结构、分页、数据一致性 | DATA |
| 安全幂等 reviewer | 注入、敏感信息、重复提交、并发 | SEC / IDEM |

对每个 reviewer，使用以下结构输出精简结果：

```markdown
### Reviewer: 业务流程 reviewer
...
| 编号 | 严重级别 | 问题 | 原因 |
|------|----------|------|------|
| R-BIZ-001 | P1 | 支付接口缺少已取消订单不可支付场景 | 涉及订单状态流转，属于核心业务反向约束 |

**建议新增/修改用例**
| 建议ID | 动作 | 所属接口 | 测试点 | 预期 | 优先级 |
|--------|------|----------|--------|------|--------|
| S-BIZ-001 | 新增 | POST /orders/{id}/pay | 已取消订单发起支付 | 400，错误码为 ORDER_CANCELLED | P1 |

**不适用项**
| 评审点 | 原因 |
|--------|------|
| CHAIN-03 | 当前接口文档未体现降级链路 |
```

### 3.3 主审汇总复核

所有 reviewer 输出完成后，由主 agent 进行汇总复核，不直接照单全收：

1. 合并重复或高度相似建议。
2. 删除明显不适用当前接口的建议。
3. 标记冲突建议，并说明需要用户确认的点。
4. 按 P0/P1/P2/P3 排序。
5. 将最终建议分为：建议新增用例、建议修改用例、不建议采纳、需用户确认。
...
| 建议ID | 来源 reviewer | 所属接口 | 测试点 | 风险原因 | 预期 | 优先级 |
|--------|---------------|----------|--------|----------|------|--------|
| S-BIZ-001 | 业务流程 reviewer | POST /orders/{id}/pay | 已取消订单发起支付 | 核心状态流转风险 | 400，错误码 ORDER_CANCELLED | P1 |
...
评审完成，请选择处理方式：
  ...
  [3] 补充要求后再评一轮 — 请输入本轮关注点（如：重点看越权/支付状态/减少参数异常）
  [4] 跳过 — 不做修改，直接进入阶段 4
```

用户选择后：

- 若选择 [1]/[2]：更新用例表，写入评审产物。
- 若选择 [3]：只围绕用户补充要求进行第二轮定向复核，不重复全量评审；第二轮后必须再次进入用户决策。
- 若选择 [4]：不修改用例，直接进入阶段 4。

写入：
- `.api-testcase-assets/history/<运行目录>/1-评审记要.md`（更新后的用例表）
- `.api-testcase-assets/history/<运行目录>/1-评审报告.md`（各 reviewer 结果 + 主审汇总）
- `.api-testcase-assets/history/<运行目录>/1-评审决策.md`（用户接受/拒绝/二轮关注点）
